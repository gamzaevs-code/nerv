...
export default function UploadVideoForm({ taskId }: { taskId: number }) {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const originalFile = recordedFile || originalFormData.get('video');
    const formData = new FormData();

    if (originalFile instanceof File) {
      if (!(await validateDuration(originalFile))) { setError('Видео должно быть не длиннее 60 секунд.'); setLoading(false); return; }
      const compressed = await maybeCompressFile(originalFile);
      formData.set('video', compressed, compressed.name || originalFile.name);
    }
  ...
  return (
    <form className="form" onSubmit={onSubmit}>
      <label>Видео<input name="video" type="file" accept="video/*" onChange={(e) => { const f = e.target.files?.[0]; if (f) setPreview(URL.createObjectURL(f)); }} /></label>
      <div className="nav-links"><button className="neon-button-outline" type="button" onClick={recording ? stopRecording : startRecording}>{recording ? 'Остановить запись' : 'Записать с камеры'}</button></div>
      {preview && <video src={preview} controls style={{ width: '100%', borderRadius: 16 }} />}
      <p className="muted">Обрезка: выберите начало/конец в системном редакторе перед загрузкой. Ограничение: 60 секунд.</p>
      <button className="neon-button" type="submit" disabled={loading}>{loading ? 'Загрузка...' : 'Загрузить видео'}</button>
